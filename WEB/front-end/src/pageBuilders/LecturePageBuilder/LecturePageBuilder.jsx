...
//   const paramsId = JSON.parse(params.value).id
...
//         // 1. Получаем расписание по lectureId
//         const scheduleRes = await axios.get(
//           `${baseUrl}/api/schedules?lectureId=${paramsId}`, 
//           { headers: { Authorization: `Bearer ${token}` } }
//         )
        
//         if (!scheduleRes.data || scheduleRes.data.length === 0) {
//           throw new Error('Расписание для лекции не найдено')
//         }
        
//         const schedule = scheduleRes.data[0]
        
//         // 2. Параллельно получаем лекцию, группу и предмет
//         const [lectureRes, groupRes, subjectRes] = await Promise.all([
//           axios.get(`${baseUrl}/api/lectures/${paramsId}`, {
//             headers: { Authorization: `Bearer ${token}` }
//           }),
//           axios.get(`${baseUrl}/api/groups/${schedule.groupId}`, {
//             headers: { Authorization: `Bearer ${token}` }
//           }),
//           axios.get(`${baseUrl}/api/subjects/${schedule.subjectId}`, {
//             headers: { Authorization: `Bearer ${token}` }
//           })
//         ])
...
//         // 3. Получаем преподавателя из лекции
//         const teacherRes = await axios.get(
//           `${baseUrl}/api/users/${lectureRes.data.teacherId}`, 
//           { headers: { Authorization: `Bearer ${token}` } }
//         )
...
//           `${baseUrl}/api/keywords?lectureId=${paramsId}`, 
//           { headers: { Authorization: `Bearer ${token}` } }
//         )
...
//         const currentUserRes = await axios.get(`${usersUrl}/$}`, {
//           mode:"no-cors",
//           headers: { Authorization: `Bearer ${token}` }
//         })
...
//         `${process.env.NEXT_PUBLIC_API_URL}/api/keywords/`,  // <-- слэш
//         {
//           lectureId: paramsId,
...
//     } catch (err) {
//       console.error('Ошибка POST /keywords/:', err.response?.status, err.response?.data)
//     }
//   }
...
const LecturePageBuilder = ({ params }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const scheduleRes = await axios.get(`${baseUrl}/api/schedules/${paramsId}`, {
          mode: 'no-cors',
        })

        const scheduleData = scheduleRes.data
        console.log(scheduleData);
        const { lecture, group } = scheduleData

        const lectureRes = await axios.get(`${baseUrl}/api/lectures/${lecture}`, {
          mode: 'no-cors',
        })
        const lectureData = lectureRes.data
        const groupRes = await axios.get(`${baseUrl}/api/groups/${group}`, {
          mode: 'no-cors',
        })
        console.log(lectureData);
        const teacherRes = await axios.get(`${baseUrl}/api/users/${lectureData.teacher}`, {
          mode: 'no-cors',
        })

        // const [subjectRes, groupRes, teacherRes] = await Promise.all([
        //   axios.get(`${baseUrl}/api/subjects/${subjectId}`, {
        //     mode: 'no-cors',
        //   }),
        //   axios.get(`${baseUrl}/api/groups/${groupId}`, {
        //     mode: 'no-cors',
        //   }),
        ...
        const keywordsRes = await axios.get(`${baseUrl}/api/keywords`, {
          mode: 'no-cors',
        })
        const keywordsData = keywordsRes.data.filter((kw) => kw.lectureId == lectureId)
  ...
  useEffect(() => {
    if (!lecture || !lecture.lecture_text) return
    let currentIndex = 0
    const interval = setInterval(() => {
      if (currentIndex <= lecture.lecture_text.length) {
        setDisplayContent(lecture.lecture_text.substring(0, currentIndex))
        currentIndex++
      } else {
        clearInterval(interval)
      }
  ...
  const handleTextSelect = () => {
    ...
    const startIndex = preSelectionRange.toString().length
    const endIndex = startIndex + selection.toString().length
    setSelection({ text: selection.toString().trim(), startIndex, endIndex })
    setIsSelecting(true)
  }
  ...
  const handleAddKeyword = async () => {
    ...
    try {
      const token = localStorage.getItem('token')
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/keywords/`,
        {
  ...
  const renderContentWithHighlights = () => {
    ...
    sortedKeywords.forEach(keyword => {
      if (keyword.startIndex > lastIndex) {
        fragments.push({ text: displayContent.substring(lastIndex, keyword.startIndex), isHighlighted: false })
      }
      fragments.push({
        text: displayContent.substring(keyword.startIndex, keyword.endIndex),
        isHighlighted: true,
    ...
    if (lastIndex < displayContent.length) {
      fragments.push({ text: displayContent.substring(lastIndex), isHighlighted: false })
    }
    return fragments.map((fragment, index) => {
      if (fragment.isHighlighted) {
        const colorClass = highlightColors[fragment.colorIndex]
  ...
  if (loading) {
    return <div className="w-full min-h-screen flex flex-col bg-mainColor"></div>
  }
  console.log(lecture.video);

  if (!lecture || !group || !teacher) {
    return (
      <div className="flex flex-col items-center justify-center h-screen">
        <h2 className="text-2xl font-bold mb-4">Данные лекции не найдены</h2>
        <Button onClick={() => router.push('/')}>Вернуться на главную</Button>
    ...
    <div className="w-full min-h-screen flex flex-col bg-mainColor">
      <div className="flex-1 container mx-auto px-4 py-8 max-w-6xl">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-8 px-6">
          <div className="space-y-2 text-white md:max-w-1/2 xs:max-w-full"></div>
        </motion.div>
        <Separator className="my-8" />
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-center mb-4 flex flex-col space-y-6">
          <h1 className="lg:text-6xl md:text-5xl 2sm:text-3xl 2xs:text-2xl font-semibold text-white">{displayTitle}</h1>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }} className="prose prose-lg max-w-none relative text-white text-2xl px-6" ref={contentRef} onMouseUp={handleTextSelect}>
          {isSelecting && !currentUser?.is_student && selection && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="absolute z-10">
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
          ...
          {currentUser?.is_student && (
            <div className="mt-6 p-4 bg-yellow-50 border-l-4 border-yellow-400 text-yellow-700">
              <p className="font-medium">
                <span className="font-bold">Информация для преподавателей:</span> Для удаления выделения наведите курсор на выделенное слово и нажмите на него.
              </p>
            </div>
          )}
        </motion.div>
